-- ==============================================================================
-- COMMON: All-in-One Complete Database Backend Setup
-- For Supabase SQL Editor execution
-- Contains: Schema, Triggers, RLS Policies, RPC Functions, and Storage Setup
-- ==============================================================================
...
CREATE TABLE IF NOT EXISTS public.borrowing_requests (
    ...
    -- Exclusion constraint: prevents overlapping approved requests for the same item
...
-- 6. COMMUNITY NEEDS (wishlist board)
...
CREATE POLICY "Community members can view item images" ON storage.objects FOR SELECT TO authenticated
USING (
...
CREATE POLICY "Users can upload item images into their own folder" ON storage.objects FOR INSERT TO authenticated
WITH CHECK (
    bucket_id = 'item-images'
...
CREATE POLICY "Users can update their own item images" ON storage.objects FOR UPDATE TO authenticated
USING (
    bucket_id = 'item-images'
    AND (storage.foldername(name))[1] = auth.uid()::text
)
...
CREATE POLICY "Users can delete their own item images" ON storage.objects FOR DELETE TO authenticated
USING (
    bucket_id = 'item-images'
    AND (storage.foldername(name))[1] = auth.uid()::text
);
